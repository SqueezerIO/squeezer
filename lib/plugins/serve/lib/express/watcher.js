...
class Watcher {
  ...
  buildCompileTree() {
    ...
    if (this.projectType === 'web') {
      ...
      srcPaths = _.union(
        this.getFileDeps(`${projectPath}/web/assets/jsx/main.jsx`),
        walkSync(`${projectPath}/web/assets/jsx`, { directories : false })
          .map(val => `${projectPath}/web/assets/jsx/${val}`)
      );
    ...
    _.forEach(microservices, (microservice) => {
      ...
      if (this.projectType === 'web') {
        srcPaths = _.union(
          this.getFileDeps(`${microservice.path}/src/web/assets/jsx/main.jsx`),
          walkSync(`${microservice.path}/src/web/assets/jsx`, { directories : false })
            .map(val => `${microservice.path}/src/web/assets/jsx/${val}`)
        );
        _.forEach(srcPaths, (src) => {
