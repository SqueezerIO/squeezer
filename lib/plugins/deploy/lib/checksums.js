...
class Checksums {
  ...
  get() {
    return new Promise((resolve, reject) => {
      ...
      const checksumFile = `${this.sqz.vars.project.buildPath}/production/checksums.json`;
      if (!fs.existsSync(checksumFile)) {
        this.sqz.cli.log.error(`It seems that your project is not compiled , use : ${colors.blue('sqz compile --production')}`);
      }
      ...
      request.get(getProjectDeploymentsEndpoint, { json : true }, (error, response, body) => {
        if (!error && response.statusCode === 200 && body.message === 'success') {
          const deployment = _.find(body.data, (val => (val.identifier === deploymentIdentifier)));
