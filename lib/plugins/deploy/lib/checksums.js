  ...
  get() {
    return new Promise((resolve, reject) => {
      ...
      if (!fs.existsSync(checksumFile)) {
        this.sqz.cli.log.error(`It seems that your project is not compiled , use : ${colors.blue('sqz compile --cloud')}`);
      }
      ...
      if (key) {
        ...
        this.sqz.cli.log.warn('By using an empty Squeezer deployment key you can\'t take the advantage of using smart deployments , all the microservices will be deployed even the ones where code din\'t changed from the previous deployment');
        resolve();
      }
    });
  }
