...
class List {
  ...
  run() {
    return new Promise((resolve) => {
      ...
      msg += colors.yellow(`\n${' '.repeat(1)}Options:\n\n`);
      msg += `${colors.green(`${' '.repeat(3)}--debug`)}` +
        `${' '.repeat((largestCmdLen - '--debug'.length) + 1)}add debugging support\n\n`;
      msg += colors.yellow(`${' '.repeat(1)}Available commands:\n\n`);

      _.forEach(singleCommands, (cmd) => {
        msg += `${colors.green(`${' '.repeat(3)}${cmd}${' '.repeat((largestCmdLen - cmd.length) + 1)}`)}` +
      ...
      _.forEach(groupedCommands, (cmds, group) => {
        msg += colors.yellow(`\n${' '.repeat(1)}${group}\n\n`);
        _.forEach(cmds, (key) => {
          msg += `${colors.green(`${' '.repeat(3)}${key}${' '.repeat((largestCmdLen - key.length) + 1)}`)}` +
