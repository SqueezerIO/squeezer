    ...
    const versionCheck    = (projectVersion, runtimeVersion) => {
      if (projectVersion !== runtimeVersion) {
        this.sqz.cli.log.warn(
          `Your ${colors.blue.bold(projectRuntime)} runtime version ( ${colors.blue.bold(runtimeVersion)} ) is different from ` +
          `the one specified in the project's config ( ${colors.blue.bold(projectVersion)} )`
        );
      }
    };
